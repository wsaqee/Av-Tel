...
//generic data colection functions

bool i2cComm::get16bitSignValue(int regMSB, int regLSB, int &data)
{
    int16_t val = 0;
    bool ret = get16bitSignValue(regMSB, regLSB, val);
    data = val;
    return ret;
}

bool i2cComm::get16bitSignValue(int regMSB, int regLSB, int16_t &data)
{
    uint8_t val = 0;
    if (!ReadByte(regMSB, &val)) return false;
    data = (int16_t)val<<8;
    if (!ReadByte(regLSB, &val)) return false;
    data += val;
    return true;
}
